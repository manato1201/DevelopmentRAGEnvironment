...
'body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;height:100vh;display:flex;flex-direction:column;max-width:900px;margin:0 auto}',
...
'  background:#1e293b;color:#e2e8f0;cursor:pointer;font-size:12px;font-family:inherit}',
...
'        <label style="font-size:.78rem;color:#94a3b8;display:flex;align-items:center;gap:5px;cursor:pointer;padding-bottom:10px">',
'        <input type="checkbox" id="new-key-admin"><span>管理者権限</span></label>',
'      </div>',
'    </div>',
'    <div style="margin-bottom:12px">',
'      <label style="font-size:.75rem;color:#64748b;display:block;margin-bottom:4px">アクセス可能なNamespace</label>',
...
'    </table>',
'  </div>',
'  <div class="admin-section" style="font-size:.8rem;color:#64748b">',
'    <h3>外部クライアント (HTTP POST) の使い方</h3>',
'    <p style="margin-bottom:8px">Unity・Houdini・Python・curl など HTTP POST が使えるクライアントであれば何でも対応しています。</p>',
'    <p style="margin-bottom:8px;color:#94a3b8">▼ リクエスト形式:</p>',
'    <pre style="background:#0f1117;padding:12px;border-radius:6px;color:#94a3b8;overflow-x:auto">',
'POST https://script.google.com/macros/s/SCRIPT_ID/exec\n',
'Content-Type: application/json\n\n',
'{\n',
'  "query": "AFURIのラーメンは？",\n',
'  "apiKey": "YOUR_32_CHAR_KEY",\n',
'  "dbKey": "all",          // 特定DBは "afuri" 等、横断検索は "all"\n',
'  "history": []            // 会話履歴 [{role:"user",text:"..."}, ...]\n',
'}',
'    </pre>',
'    <p style="margin-top:8px;margin-bottom:4px;color:#94a3b8">▼ レスポンス:</p>',
'    <pre style="background:#0f1117;padding:12px;border-radius:6px;color:#94a3b8;overflow-x:auto">',
'{\n',
'  "status": "ok",\n',
'  "answer": "回答テキスト",\n',
...
'    エラー時: <code>status</code> が <code>"auth_error"</code>（キー無効）または <code>"forbidden"</code>（DB権限なし）</p>',
'  </div>',
'</div>',
'</div>',
...
'  if (target) {',
'    target.style.display = (tab === "chat") ? "flex" : "flex";',
'    if (tab === "chat" || tab === "graph") target.style.flexDirection = "column";',
