...
// 発生した（2026-08-27）。
// 当初はPromise.raceだけで期限を区切るwithTimeout()を使っていたが、その弱点
// （2026-08-27に実機で判明）: Promise.raceは「待つのをやめる」だけで、負けた方の
// Promiseの中身（実際のfetch）は裏で動き続ける。1ファイルをタイムアウトさせて次に
// 進んでも、その裏で継続中のDriveダウンロード等がWorker全体の実行時間を消費し続け、
// 結果としてバッチ全体がCloudflareエッジに打ち切られ503になる、という形で問題が
// 再発した。そのためwithTimeout()は削除し、fetch自体を本当にAbortController.abort()で
// 中断できる下記のwithAbortTimeout()に一本化した。
