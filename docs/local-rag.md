...
初回起動時は管理者ユーザーが1人も登録されていないため、先に管理者を作成してAPIキーを取得します（このAPIキーが無いと `/query` 等の認証必須エンドポイントは使えません）。

```powershell
# 1. namespace用フォルダを作成（未作成の場合）
New-Item -ItemType Directory -Path localRAG\tool_docs, localRAG\research, localRAG\team_notes, localRAG\personal_notes, localRAG\game_info -Force

# 2. 管理者ユーザーを作成（APIキーが標準出力に1度だけ表示される。必ず控えること）
uv run python scripts\auth_manager.py create-admin --name "Admin"
```
...
Unity・Houdini の RAG Chatbot UI から `localhost:8766` に接続して使用します（詳細は「[7. UI実装詳細](#7-ui実装詳細)」参照）。`/admin` は `scripts/static/admin.html`、`/ui` は `scripts/static/user_ui.html` をそのまま配信する管理者画面・ユーザーチャット画面です。動作確認：
...
### markitdown のインポートエラー（document_pipeline.py）

`uv sync` を実行してから、リポジトリのルートで `uv run python scripts\document_pipeline.py add ...` のように `uv run` 経由で実行してください。仮想環境外から直接 `python` で実行すると `markitdown` が見つからずエラーになります。

### チャンクが多すぎる（PDF が長い）

`--max-chunk` オプションでチャンクサイズの上限を引き上げてください（例: `--max-chunk 1200`）。

### 日本語のチャンクが途中で切れる

見出し（`#`/`##`/`###`）を増やして文書を細かく区切ってください。見出しベース分割は見出し単位を最優先で尊重するため、見出しが少ないと長いセクションがオーバーラップ分割の境界で機械的に切られやすくなります。
