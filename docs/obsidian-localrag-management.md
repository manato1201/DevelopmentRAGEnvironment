...
> **⚠️ 過去の設計資料（歴史的資料）:** 2026-06-30 時点で `mcp-rag-server` への依存は解消され、検索エンジンはこのリポジトリに直接ベンダー統合されています（`scripts/rag_service.py` 等）。本ドキュメントが前提とする WSL + `~/mcp-rag-server` 構成・`MCP_RAG_DIR` 変数は現行の Windows ネイティブ構成では使われません。現行の自動インデックス化は `scripts/auto_index.py`（このリポジトリ内で完結）を参照し、セットアップ全体は [docs/local-rag-setup.md](local-rag-setup.md) を参照してください。設計思想（Obsidian vault を管理インターフェースにする発想自体）は引き続き有効です。
