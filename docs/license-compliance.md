...
### 義務
- **配布時にのみ**: `LICENSE` ファイル（著作権表示）を同梱すること
- 内部ツールとして使うだけなら配布不要 → 義務なし

### 依存パッケージのライセンス

mcp-rag-server が依存するパッケージはすべて許容的ライセンス:
...
mcp-rag-server が内部で使う `mcp[cli]` ライブラリ。MIT なので商用利用・改造ともに問題なし。配布時は著作権表示を維持するだけでよい。
...
### 配布時に必要な著作権表示（mcp-rag-server を配布する場合）
