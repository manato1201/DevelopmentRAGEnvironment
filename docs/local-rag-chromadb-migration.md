...
> **⚠️ 過去の移行記録（歴史的資料）:** 2026-06-30 時点で `mcp-rag-server` への依存は完全に解消され、検索エンジンはこのリポジトリに直接ベンダー統合されています。本ドキュメントはその前段階（pgvector→ChromaDB移行時点）の記録であり、`git clone` 等の手順は現在は不要です。現行のセットアップ手順は [docs/local-rag-setup.md](local-rag-setup.md) を参照してください。
