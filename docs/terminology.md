...
クエリをそのまま埋め込むのではなく、まず「もしこの質問への理想的な回答があったとしたら？」という仮説文書をLLMに生成させ、その仮説文書の埋め込みをクエリ埋め込みと加重平均して検索するテクニック。語彙ミスマッチ（ユーザーの言葉と文書の言葉が違う問題）を大幅に解消できる。技術ドメイン（houdini21/tool_docs/research）はクエリ40%＋仮説文書60%で混合するが、`afuri`/`braintq`/`fourteen`のような固有事実ドメインはクエリ80%＋仮説20%（`FACT_HEAVY_DOMAINS`参照）。
...
### FACT_HEAVY_DOMAINS
`gas_cloud_rag.js`で定義される配列 `['afuri', 'braintq', 'fourteen']`。LLMが実店舗名・価格などの固有事実を知らないドメインを示す。本番運用でAFURI「辛紅ラーメン」のような固有名詞についてGeminiがハルシネーションした仮説文書を生成し検索精度が84.4%→79.7%に低下する問題が発覚したため導入。該当ドメインは`hydeExpand_`内でクエリ側の重みを0.8に引き上げる。
...
### afuri / braintq / fourteen（DBキー）
houdini21と並ぶCloud RAGのDBキー。`afuri`=飲食店（AFURI阿夫利）のメニュー・店舗情報、`braintq`=サービス・施設情報、`fourteen`=ゴルフ場・アパレル情報を扱う名前空間。いずれも実店舗・実在ブランドの固有事実を扱うため`FACT_HEAVY_DOMAINS`に含まれ、HyDEの仮説文書重みが抑制される。
...
### rag_local_bridge.py
Local RAGのHTTPブリッジサーバー。Port 8766で起動。内部の`LocalRAGClient`が`rag_service.py`をプロセス内で直接importして呼び出す（旧版はmcp-rag-serverをサブプロセス起動しMCP JSON-RPC over stdioで通信していたが、2026-06-30の独立化で廃止）。  
主要エンドポイント：
| Method | Path | 機能 |
|---|---|---|
| GET | `/health` | 生存確認・インデックス件数取得 |
| POST | `/query` | RAG検索+LLM回答 |
| GET | `/graph` | ドキュメント関係グラフJSON取得 |
| POST | `/api/score` | 理解度スコア更新 |
| GET/POST | `/api/llm-backend` | LLMバックエンド確認/変更 |
| GET | `/admin/audit` | 監査ログ取得 |

### LocalRAGエンジンのvendoring（2026-06-30）
LocalRAGは以前、外部リポジトリ`mcp-rag-server`（karaage0703氏、MIT）を別途cloneして依存していたが、2026-06-30にその検索エンジンコードをこのリポジトリへ直接取り込み（vendoring）、外部依存を解消した。取り込まれたのは`document_processor.py`・`embedding_generator.py`・`rag_service.py`・`mcp_server.py`・`rag_mcp_tools.py`・`rag_mcp_server.py`の6ファイル。MITライセンスのため改変・再配布・商用利用は自由（詳細は`docs/license-compliance.md`参照）。

### rag_service.py
LocalRAGの検索エンジン統合層。`document_processor.py`（ファイル読込・チャンク分割）・`embedding_generator.py`（sentence-transformersラッパー）・`vector_database.py`（ChromaDB+BM25）を束ねる`RAGService`クラスを提供。`create_rag_service_from_env()`で環境変数から初期化する（`.env`非使用、`os.environ.get()`のみ）。

### LocalRAGClient
`rag_local_bridge.py`内のクラス。`rag_service.py`の`RAGService`をプロセス内で直接呼び出し、検索結果を旧mcp-rag-server時代のテキスト形式（"ファイル:"行を含む）と互換のフォーマットで返す。namespaceフィルタ処理が引き続きこの形式に依存しているため。

### rag_cli.py
LocalRAGのインデックス化CLI。`uv run python scripts/rag_cli.py index|clear|count [--incremental]`。旧mcp-rag-server側の`uv run python -m src.cli index`を置き換える。

### rag_mcp_server.py / rag_mcp_tools.py
Claude Desktopに直接MCPサーバーとして登録するためのエントリーポイント（`rag_mcp_server.py`）と、search/get_document_countツールハンドラ（`rag_mcp_tools.py`）。`mcp_server.py`（JSON-RPC over stdioのプロトコル実装）と組み合わせて動作し、旧mcp-rag-serverの`src/main.py`相当を置き換える。
...
### WSL2
Windows Subsystem for Linux 2。2026-06-30のmcp-rag-server独立化以降、`rag_local_bridge.py`はWindowsネイティブで動作しWSL2は不要になった。WSL2構成は移行前の歴史的な記録（`docs/local-rag.md` §9参照）。

### Docker
将来的なコンテナ化を想定。現状はWindowsネイティブPython環境（uv管理）で動作し、Dockerは不使用。
